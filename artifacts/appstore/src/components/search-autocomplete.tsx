...
export function SearchAutocomplete({
  ...
  const { data: suggestions, isLoading: loadingSuggestions } = useListApps(
    { search: debouncedQuery, limit: 7 } as any,
    { query: { enabled: debouncedQuery.length >= 1 } } as any
  );

  const { data: trendingApps } = useListApps(
    { trending: true, limit: 5 } as any
  );
  ...
  const showTrending = !showSuggestions && (trendingApps ?? []).length > 0;
